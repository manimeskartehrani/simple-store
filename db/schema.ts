...
export const section = pgTable('section', {
    ...
    courseId: integer('course_id').references(() => course.id, { onDelete: 'cascade' }).notNull()
})
...
export const subsection = pgTable('subsection', {
    ...
    videLength: integer('videoLength'),
    markdownFilePath: text('markdownFilePath'),
    sectionId: integer('section_id').references(() => section.id, { onDelete: 'cascade' }).notNull()
})

export type SelectSubsection = typeof subsection.$inferSelect
export type InsertSubsection = typeof subsection.$inferInsert

export const subsectionRelations = relations(subsection, ({ one }) => ({
    section: one(section, {
